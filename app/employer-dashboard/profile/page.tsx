...
export default function EmployerProfilePage() {
  ...
  const accountDetails = useMemo(
    () => [
      { label: "Signed-in email", value: profile?.login_email },
      { label: "Company / Restaurant", value: profile?.company_name },
      { label: "Contact name", value: profile?.contact_name },
      { label: "Phone", value: profile?.phone },
      { label: "Support / contact email", value: profile?.support_email },
      { label: "Address", value: profile?.address },
      { label: "City", value: profile?.city },
      { label: "State", value: profile?.state },
      { label: "ZIP / Postal code", value: profile?.postal_code },
      { label: "Signup first name", value: profile?.first_name },
      { label: "Signup last name", value: profile?.last_name },
      { label: "Signup job title", value: profile?.job_title },
      { label: "Signup jobs open", value: profile?.jobs_open },
    ],
    [profile],
  );
      ...
      <div style={{ maxWidth: 1140, margin: "0 auto", padding: "0 18px" }}>
        ...
        <div className="rn-profile-grid">
          <section style={homeCardStyle}>
            <h2 className="rn-profile-section-title">Account Information</h2>
            <p className="rn-profile-section-copy">
              This includes your login email, signup metadata, and the latest saved employer profile details. Your login email is not editable here.
            </p>
            <div className="rn-profile-details-grid">
              {accountDetails.map((item) => (
                <article key={item.label} className="rn-profile-detail-card">
                  <p>{item.label}</p>
