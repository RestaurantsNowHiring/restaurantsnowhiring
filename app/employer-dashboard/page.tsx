...
type EmployerOwner = { userId: string; email: string; accountId?: string | null; ownerUserId?: string; ownerEmail?: string; accessScope?: EmployerAccessScope | null; assignedStoreIds?: string[] };
...
type EmployerAccountMembership = { accountId: string; accountName: string; locationName: string | null; accessScope: EmployerAccessScope; assignedStoreIds: string[]; role: EmployerRole; status?: string; invitationPending?: boolean };
type EmployerAccess = { role: EmployerRole; accountId: string | null; accountName: string | null; restaurantBrandName: string | null; locationName: string | null; accessScope: EmployerAccessScope; assignedStoreIds: string[]; memberships: EmployerAccountMembership[]; ownerUserId: string; ownerEmail: string; canManageProfile: boolean; canManageBilling: boolean; canManageJobs: boolean; canViewCandidates: boolean; canUpdateCandidateStatuses: boolean; canManageTeam: boolean; canManageNotificationRouting: boolean; };
...
const JOB_QUERY_VARIANTS: JobsQueryVariant[] = [
  {
    fields: "id,title,restaurant_name,city,state,active,status,created_at,views,employer_user_id,employer_email,employer_account_id,candidate_notification_email,employer_store_id,candidate_notification_emails",
    includesStatus: true,
    includesViews: true,
  },
  {
    fields: "id,title,restaurant_name,city,state,active,status,created_at,employer_user_id,employer_email,employer_account_id,candidate_notification_email,employer_store_id,candidate_notification_emails",
    includesStatus: true,
    includesViews: false,
  },
];
  ...
  useEffect(() => {
    ...
    async function loadDashboard() {
      ...
      const currentOwner = {
        ...
        accessScope: access?.accessScope ?? null,
      ...
      const visibleJobRows = jobsResult.liveJobs.filter((job) => canUserAccessJob({ email, accessScope: access?.accessScope, assignedStoreIds: access?.assignedStoreIds }, access?.role ?? "account_owner", job));
      ...
      const hydratedJobs: DashboardJob[] = visibleJobRows.map((job) => {
        ...
        return {
          ...
          employer_store_id: typeof job.employer_store_id === "string" ? job.employer_store_id : null,
