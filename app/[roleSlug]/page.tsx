...
function getStateCompanies(jobs: RoleJob[]) {
  return Array.from(
    new Set(
      jobs
        .map((job) => job.restaurant_name?.trim())
...
function StateLandingPageContent({
  ...
  const companies = getStateCompanies(jobs);
  const featuredJobs = jobs.slice(0, 6);
  const allStateJobsHref = `/jobs?state=${encodeURIComponent(state.code)}`;
  const statCards = [
    { label: "Active Jobs", value: jobs.length },
    { label: "Cities Hiring", value: cities.length },
    { label: "Restaurant Companies Hiring", value: companies.length },
  ];
      ...
      <section style={{ width: "100%", padding: "18px 0 14px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 18px" }}>
          ...
          <div
            ...
            aria-label={`${state.name} restaurant job stats`}
          >
            {statCards.map((stat) => (
              <div
                key={stat.label}
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 18px" }}>
          ...
          >
            ...
            >
              ...
              <Link href={allStateJobsHref} className="hero-button rn-btn-secondary" style={homeSecondaryButton}>
                ...
                return (
                  ...
                  >
                    <div style={{ minWidth: 0 }}>
                      ...
                      <div style={{ color: "rgba(0,0,0,.72)", fontWeight: 750, marginTop: 4 }}>
                        {job.restaurant_name} — {job.city}, {job.state}
                      </div>
                      {(type || pay) && (
                        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 8 }}>
                          {type && <span style={stateJobChipStyle}>{type}</span>}
                          {pay && <span style={stateJobChipStyle}>{pay}</span>}
                        </div>
                      )}
                    </div>
                    <Link
