...
function setup({ connection = {}, jobs = [], remote = [], parseError = false, updateErrorIds = [], mappings = [], hydrateJobs } = {}) {
  ...
  const provider = { parseJobs: async () => { if (parseError) throw new Error("secret"); return remote; }, ...(hydrateJobs ? { hydrateJobs } : {}) };
...
test("provider hydration during sync covers only imported open jobs and preserves existing descriptions on hydration failure", async () => {
  const hydrateCalls = [];
  const remote = [providerJob("1", { descriptionHtml: undefined }), providerJob("new", { descriptionHtml: undefined }), providerJob("closed-imported", { descriptionHtml: undefined })];
  const db = setup({
    jobs: [existingJob("1"), existingJob("missing")],
    remote,
    hydrateJobs: async ({ jobs }) => {
      hydrateCalls.push(jobs.map((item) => item.externalId));
      return [{ status: "ready", job: providerJob("1", { descriptionHtml: "<p>Hydrated</p>" }) }];
    },
  });
  const result = await sync({ connectionId: "x" }, db.dependencies);
  assert.deepEqual(hydrateCalls, [["1"]]);
  assert.equal(result.NewAvailable.length, 2);
  assert.equal(db.calls.updates.find((item) => item.id === "rnh-1").payload.description, "<p>Hydrated</p>");
  assert.equal(db.calls.updates.find((item) => item.id === "rnh-missing").payload.ats_inactive_reason, "closed_in_ats");

  const failed = setup({
    jobs: [existingJob("1", { description: "Keep me" })],
    remote: [providerJob("1", { descriptionHtml: undefined })],
    hydrateJobs: async () => [{ status: "unavailable", providerKey: "greenhouse", externalId: "1" }],
  });
  const failedResult = await sync({ connectionId: "x" }, failed.dependencies);
  assert.equal(failedResult.Failed.length, 1);
  assert.equal(failed.calls.updates.length, 0);
});
