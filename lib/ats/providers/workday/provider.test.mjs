import assert from "node:assert/strict";
import { test } from "node:test";
import { readFileSync } from "node:fs";
import { dirname, resolve } from "node:path";
import { fileURLToPath } from "node:url";
import ts from "typescript";

const providerPath = resolve(
  dirname(fileURLToPath(import.meta.url)),
  "provider.ts",
);
const registryPath = resolve(
  dirname(fileURLToPath(import.meta.url)),
  "../registry.ts",
);

function loadProvider(patches = {}) {
  let source = readFileSync(providerPath, "utf8")
    .replace('import "server-only";\n\n', "")
    .replace(/import type \{[^}]+\} from "\.\.\/\.\.\/types";\n\n/s, "");
  for (const [pattern, replacement] of Object.entries(patches))
    source = source.replace(pattern, replacement);
  const { outputText } = ts.transpileModule(source, {
    compilerOptions: {
      module: ts.ModuleKind.CommonJS,
      target: ts.ScriptTarget.ES2022,
    },
  });
  const testModule = { exports: {} };
  new Function("exports", "require", "module", outputText)(
    testModule.exports,
    () => {
      throw new Error("Unexpected require");
    },
    testModule,
  );
  return testModule.exports;
}

const {
  workdayProvider,
  WORKDAY_DETAIL_CONCURRENCY,
  WORKDAY_LISTING_CONCURRENCY,
  WORKDAY_LISTING_MAX_ATTEMPTS,
  WORKDAY_LISTING_TIMEOUT_MS,
  WORKDAY_SENTINEL_LOOKAHEAD_PAGES,
  WORKDAY_MAX_CUMULATIVE_BYTES,
  WORKDAY_MAX_JOBS,
  WORKDAY_MAX_PAGES,
  WORKDAY_MAX_RESPONSE_BYTES,
  WORKDAY_MAX_TOTAL_DRIFT,
  WORKDAY_PAGE_SIZE,
  WORKDAY_REQUEST_TIMEOUT_MS,
} = loadProvider();
const careersPage = (url) => ({ url });
const json = (body, init = {}) =>
  Response.json(body, {
    headers: { "content-type": "application/json" },
    ...init,
  });

async function detectionFor(url) {
  const { matched, providerKey, confidence } = await workdayProvider.detect(
    careersPage(url),
  );
  return { matched, providerKey, confidence };
}

function installWorkdayFetch(t, handler) {
  const previousFetch = globalThis.fetch;
  t.after(() => {
    globalThis.fetch = previousFetch;
  });
  globalThis.fetch = handler;
}

test("detect supports locale-prefixed, non-locale, and full job-detail Workday URLs", async () => {
  for (const url of [
    "https://tenant.wd5.myworkdayjobs.com/Site",
    "https://tenant.wd5.myworkdayjobs.com/en-US/Site",
    "https://tenant.wd5.myworkdayjobs.com/en-US/Site/job/Location/Title_R123",
    "https://tenant.wd5.myworkdayjobs.com/Site/job/Location/Title_R123",
    "https://tenant.wd12.myworkdayjobs.eu/Site",
  ])
    assert.deepEqual(await detectionFor(url), {
      matched: true,
      providerKey: "workday",
      confidence: "high",
    });
});

test("detect rejects unsupported pages and nested paths where site cannot be determined confidently", async () => {
  for (const url of [
    "https://tenant.wd5.myworkdayjobs.com",
    "https://tenant.wd5.myworkdayjobs.com/en-US",
    "https://tenant.wd5.myworkdayjobs.com/en-US/Site/search/results",
    "https://example.com/Site",
    "ftp://tenant.wd5.myworkdayjobs.com/Site",
    "https://user:pass@tenant.wd5.myworkdayjobs.com/Site",
    "https://tenant.foo.myworkdayjobs.com/Site",
  ])
    assert.deepEqual(await detectionFor(url), {
      matched: false,
      providerKey: null,
      confidence: null,
    });
});

test("paginated preview fetches only the requested listing page and never fetches detail", async (t) => {
  const calls = [];
  installWorkdayFetch(t, async (url, init) => {
    calls.push({ url, method: init.method, body: init.body });
    const { offset, limit, searchText } = JSON.parse(init.body);
    assert.equal(offset, 20);
    assert.equal(limit, WORKDAY_PAGE_SIZE);
    assert.equal(searchText, "");
    return json({ total: 10_000, jobPostings: Array.from({ length: 20 }, (_, index) => ({
      title: `Job ${offset + index}`, jobReqId: `R${offset + index}`, externalPath: `/job/Location/Job_R${offset + index}`,
    })) });
  });

  const page = await workdayProvider.listJobsPage({ careersPage: careersPage("https://tenant.wd5.myworkdayjobs.com/Site"), offset: 20, limit: 20 });
  assert.equal(page.jobs.length, 20);
  assert.equal(page.nextOffset, 40);
  assert.equal(page.hasMore, true);
  assert.equal(calls.length, 1);
  assert.equal(calls[0].method, "POST");
});

test("paginated preview stops on a short page and repeated pages reach every job", async (t) => {
  const offsets = [];
  installWorkdayFetch(t, async (_url, init) => {
    const { offset } = JSON.parse(init.body);
    offsets.push(offset);
    const count = offset < 40 ? 20 : 7;
    return json({ total: 47, jobPostings: Array.from({ length: count }, (_, index) => ({
      title: `Job ${offset + index}`, jobReqId: `R${offset + index}`, externalPath: `/job/Location/Job_R${offset + index}`,
    })) });
  });
  const jobs = [];
  let offset = 0;
  do {
    const page = await workdayProvider.listJobsPage({ careersPage: careersPage("https://tenant.wd5.myworkdayjobs.com/Site"), offset, limit: 20 });
    jobs.push(...page.jobs);
    if (!page.hasMore) { assert.equal(page.nextOffset, null); break; }
    offset = page.nextOffset;
  } while (true);
  assert.equal(jobs.length, 47);
  assert.deepEqual(offsets, [0, 20, 40]);
});

test("parseJobs derives the correct site from a full job URL and separates API detail URL from public apply URL", async (t) => {
  const calls = [];
  installWorkdayFetch(t, async (url, init) => {
    calls.push({ url, method: init.method });
    if (init.method === "POST") {
      assert.equal(
        url,
        "https://tenant.wd5.myworkdayjobs.com/wday/cxs/tenant/Site/jobs",
      );
      const offset = JSON.parse(init.body).offset;
      return json({
        total: 1,
        jobPostings:
          offset === 0
            ? [
                {
                  title: "Cook",
                  externalPath: "/job/Location/Title_R123",
                  postedOn: "2026-07-01",
                },
              ]
            : [],
      });
    }
    assert.equal(
      url,
      "https://tenant.wd5.myworkdayjobs.com/wday/cxs/tenant/Site/job/Location/Title_R123",
    );
    return json({
      jobPostingInfo: {
        title: "Cook",
        jobReqId: "R123",
        jobDescription: "<p>Cook</p>",
        startDate: "2099-01-01",
      },
    });
  });
  const parsed = await workdayProvider.parseJobs(
    careersPage(
      "https://tenant.wd5.myworkdayjobs.com/en-US/Site/job/Location/Title_R123",
    ),
  );
  assert.equal(
    parsed[0].applyUrl,
    "https://tenant.wd5.myworkdayjobs.com/en-US/Site/job/Location/Title_R123",
  );
  assert.equal(parsed[0].sourceUrl, parsed[0].applyUrl);
  assert.equal(parsed[0].updatedAt, "2026-07-01");
  assert.equal(calls.filter((call) => call.method === "GET").length, 1);
});

test("parseJobs preserves non-locale public apply paths", async (t) => {
  installWorkdayFetch(t, async (_url, init) =>
    init.method === "POST"
      ? json({
          jobPostings: [{ title: "Host", externalPath: "/job/Towson/Host_R1" }],
        })
      : json({
          jobPostingInfo: { jobReqId: "R1", jobDescription: "<p>Host</p>" },
        }),
  );
  const parsed = await workdayProvider.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
  );
  assert.equal(
    parsed[0].applyUrl,
    "https://tenant.wd5.myworkdayjobs.com/Site/job/Towson/Host_R1",
  );
});

test("malicious externalPath values are rejected without cross-host fetches or partial results", async (t) => {
  for (const externalPath of [
    "https://evil.example/job/X/Y_R1",
    "//evil.example/job/X/Y_R1",
    "/job/../Admin/Y_R1",
    "/not-job/X/Y_R1",
    "/job/X",
  ]) {
    let getCalled = false;
    installWorkdayFetch(t, async (_url, init) => {
      if (init.method === "GET") getCalled = true;
      return init.method === "POST"
        ? json({ jobPostings: [{ title: "Bad", externalPath }] })
        : json({ jobPostingInfo: {} });
    });
    await assert.rejects(
      () =>
        workdayProvider.parseJobs(
          careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
        ),
      /invalid external path|listing ended/,
    );
    assert.equal(getCalled, false);
  }
});

test("normal pagination completes and overlapping pages are deduplicated in first-seen order", async (t) => {
  const detailUrls = [];
  installWorkdayFetch(t, async (url, init) => {
    if (init.method === "GET") {
      detailUrls.push(url);
      return json({
        jobPostingInfo: {
          jobReqId: url.split("_").at(-1),
          jobDescription: "<p>Ok</p>",
        },
      });
    }
    const offset = JSON.parse(init.body).offset;
    if (offset === 0)
      return json({
        total: 21,
        jobPostings: Array.from({ length: 20 }, (_, i) => ({
          title: `Job ${i}`,
          externalPath: `/job/Loc/Job_${i}`,
        })),
      });
    if (offset > 20) return json({ total: 21, jobPostings: [] });
    return json({
      total: 21,
      jobPostings: [
        { title: "Duplicate", externalPath: "/job/Loc/Job_19" },
        { title: "Job 20", externalPath: "/job/Loc/Job_20" },
      ],
    });
  });
  const parsed = await workdayProvider.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
  );
  assert.equal(parsed.length, 21);
  assert.equal(parsed.at(-1).externalId, "20");
  assert.equal(new Set(detailUrls).size, 21);
});

test("listing requests use the public Workday page size and advance offsets", async (t) => {
  const requests = [];
  installWorkdayFetch(t, async (_url, init) => {
    if (init.method === "GET")
      return json({
        jobPostingInfo: { jobReqId: "R", jobDescription: "<p>Ok</p>" },
      });
    const body = JSON.parse(init.body);
    requests.push(body);
    const start = body.offset;
    const remaining = Math.max(0, 41 - start);
    return json({
      total: 41,
      jobPostings: Array.from({ length: Math.min(20, remaining) }, (_, i) => ({
        title: `Job ${start + i}`,
        externalPath: `/job/L/J_${start + i}`,
      })),
    });
  });

  const parsed = await workdayProvider.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
  );

  assert.equal(requests[0].limit, 20);
  assert.deepEqual(
    requests.map((request) => request.offset),
    [0, 20, 40, 60],
  );
  assert.equal(parsed.length, 41);
});

function installPagedWorkdayFetch(t, pages) {
  installWorkdayFetch(t, async (url, init) => {
    if (init.method === "GET")
      return json({
        jobPostingInfo: {
          title: "Detail",
          jobReqId: url.split("_").at(-1),
          jobDescription: "<p>Ok</p>",
        },
      });
    const offset = JSON.parse(init.body).offset;
    const page = pages.get(offset) ?? { total: 0, count: 0 };
    return json({
      total: page.total,
      jobPostings:
        page.jobs ??
        Array.from({ length: page.count }, (_, i) => ({
          title: `Job ${offset + i}`,
          externalPath: `/job/L/J_${offset + i}`,
        })),
    });
  });
}

test("small Workday total increase still requires and succeeds on a short page", async (t) => {
  installPagedWorkdayFetch(
    t,
    new Map([
      [0, { total: 21, count: 20 }],
      [20, { total: 22, count: 2 }],
    ]),
  );
  const parsed = await workdayProvider.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
  );
  assert.equal(parsed.length, 22);
});

test("small Workday total decrease succeeds on a short page", async (t) => {
  installPagedWorkdayFetch(
    t,
    new Map([
      [0, { total: 25, count: 20 }],
      [20, { total: 22, count: 2 }],
    ]),
  );
  const parsed = await workdayProvider.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
  );
  assert.equal(parsed.length, 22);
});

test("several Workday total changes within the drift bound succeed", async (t) => {
  installPagedWorkdayFetch(
    t,
    new Map([
      [0, { total: 57, count: 20 }],
      [20, { total: 60, count: 20 }],
      [40, { total: 55, count: 15 }],
    ]),
  );
  const parsed = await workdayProvider.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
  );
  assert.equal(parsed.length, 55);
});

test("excessive Workday total drift fails as unstable", async (t) => {
  installPagedWorkdayFetch(
    t,
    new Map([
      [0, { total: 20, count: 20 }],
      [20, { total: 20 + WORKDAY_MAX_TOTAL_DRIFT + 1, count: 1 }],
    ]),
  );
  await assert.rejects(
    () =>
      workdayProvider.parseJobs(
        careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
      ),
    /drift limit/,
  );
});

test("full page at the reported total is not treated as complete", async (t) => {
  const offsets = [];
  installWorkdayFetch(t, async (_url, init) => {
    if (init.method === "GET")
      return json({
        jobPostingInfo: { jobReqId: "R", jobDescription: "<p>Ok</p>" },
      });
    const offset = JSON.parse(init.body).offset;
    offsets.push(offset);
    return json({
      total: 20,
      jobPostings:
        offset === 0
          ? Array.from({ length: 20 }, (_, i) => ({
              title: `Job ${i}`,
              externalPath: `/job/L/J_${i}`,
            }))
          : [],
    });
  });
  const parsed = await workdayProvider.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
  );
  assert.equal(parsed.length, 20);
  assert.deepEqual(offsets, [0, 20]);
});

test("short-page validation uses raw rows instead of deduplicated listings", async (t) => {
  installPagedWorkdayFetch(
    t,
    new Map([
      [0, { total: 21, count: 20 }],
      [
        20,
        {
          total: 21,
          jobs: [{ title: "Duplicate", externalPath: "/job/L/J_19" }],
        },
      ],
    ]),
  );
  const parsed = await workdayProvider.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
  );
  assert.equal(parsed.length, 20);
});

test("page limit fails rather than treating max full pages as complete", async (t) => {
  const offsets = [];
  installWorkdayFetch(t, async (url, init) => {
    if (init.method === "GET")
      return json({
        jobPostingInfo: {
          jobReqId: url.split("_").at(-1),
          jobDescription: "<p>Ok</p>",
        },
      });
    const { limit, offset } = JSON.parse(init.body);
    offsets.push(offset);
    assert.equal(limit, 20);
    return json({
      total: 10000,
      jobPostings: Array.from({ length: 20 }, (_, i) => ({
        title: `Job ${offset + i}`,
        externalPath: `/job/L/J_${offset + i}`,
      })),
    });
  });

  await assert.rejects(
    () =>
      workdayProvider.parseJobs(
        careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
      ),
    /safe page limit/,
  );
  assert.equal(offsets.length, 1);
  assert.equal(offsets[0], 0);
});

test("later Workday zero total with full jobs is ignored and pagination continues to a short page", async (t) => {
  const offsets = [];
  const logs = captureConsoleError(t);
  installWorkdayFetch(t, async (url, init) => {
    if (init.method === "GET")
      return json({
        jobPostingInfo: {
          title: "Detail",
          jobReqId: url.split("_").at(-1),
          jobDescription: "<p>Ok</p>",
        },
      });
    const offset = JSON.parse(init.body).offset;
    offsets.push(offset);
    const remaining = Math.max(0, 1682 - offset);
    const pageSize = Math.min(20, remaining);
    return json({
      total: offset === 0 ? 1682 : 0,
      jobPostings: Array.from({ length: pageSize }, (_, i) => ({
        title: `Job ${offset + i}`,
        externalPath: `/job/L/J_${offset + i}`,
      })),
    });
  });

  const parsed = await workdayProvider.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
  );

  assert.equal(parsed.length, 1682);
  assert.equal(offsets.length, 86);
  assert.deepEqual(offsets.slice(0, 3), [0, 20, 40]);
  assert.equal(offsets.at(-1), 1700);
  assert.deepEqual(logs, []);
});

test("later Workday zero total does not reset tracked totals or cause total instability", async (t) => {
  const logs = captureConsoleError(t);
  installPagedWorkdayFetch(
    t,
    new Map([
      [0, { total: 1682, count: 20 }],
      [20, { total: 0, count: 20 }],
      [40, { total: 1682 + WORKDAY_MAX_TOTAL_DRIFT + 1, count: 1 }],
    ]),
  );

  await assert.rejects(
    () =>
      workdayProvider.parseJobs(
        careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
      ),
    /drift limit/,
  );

  assert.deepEqual(logs, [
    [
      {
        provider: "workday",
        stage: "listing",
        failureCode: "pagination_total_unstable",
        firstReportedTotal: 1682,
        minimumReportedTotal: 1682,
        maximumReportedTotal: 1682 + WORKDAY_MAX_TOTAL_DRIFT + 1,
        latestReportedTotal: 1682 + WORKDAY_MAX_TOTAL_DRIFT + 1,
        pagesRequested: 86,
        rawRowsRetrieved: 41,
      },
    ],
  ]);
});

test("first Workday page total zero with no jobs is a complete empty board", async (t) => {
  const requests = [];
  installWorkdayFetch(t, async (_url, init) => {
    requests.push(JSON.parse(init.body).offset);
    return json({ total: 0, jobPostings: [] });
  });

  const parsed = await workdayProvider.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
  );

  assert.deepEqual(parsed, []);
  assert.deepEqual(requests, [0]);
});

test("first Workday page total zero with jobs is treated as unavailable and does not truncate", async (t) => {
  const offsets = [];
  installWorkdayFetch(t, async (url, init) => {
    if (init.method === "GET")
      return json({
        jobPostingInfo: {
          title: "Detail",
          jobReqId: url.split("_").at(-1),
          jobDescription: "<p>Ok</p>",
        },
      });
    const offset = JSON.parse(init.body).offset;
    offsets.push(offset);
    return json({
      total: 0,
      jobPostings:
        offset === 0
          ? Array.from({ length: 20 }, (_, i) => ({
              title: `Job ${i}`,
              externalPath: `/job/L/J_${i}`,
            }))
          : offset === 20
            ? [{ title: "Job 20", externalPath: "/job/L/J_20" }]
            : [],
    });
  });

  const parsed = await workdayProvider.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
  );

  assert.equal(parsed.length, 21);
  assert.deepEqual(
    [...offsets].sort((a, b) => a - b),
    Array.from(
      { length: WORKDAY_SENTINEL_LOOKAHEAD_PAGES + 1 },
      (_, index) => index * WORKDAY_PAGE_SIZE,
    ),
  );
});

test("stale larger Workday total after ignored zero does not reject contiguous terminal short page", async (t) => {
  installPagedWorkdayFetch(
    t,
    new Map([
      [0, { total: 180, count: 20 }],
      [20, { total: 0, count: 20 }],
      [40, { total: 0, count: 1 }],
    ]),
  );

  const parsed = await workdayProvider.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
  );
  assert.equal(parsed.length, 41);
});

test("repeated postings fetch detail once", async (t) => {
  let details = 0;
  installWorkdayFetch(t, async (_url, init) => {
    if (init.method === "GET") {
      details += 1;
      return json({ jobPostingInfo: { jobReqId: "R1" } });
    }
    const offset = JSON.parse(init.body).offset;
    return json({
      total: 2,
      jobPostings:
        offset === 0
          ? [
              { title: "A", externalPath: "/job/L/A_R1" },
              { title: "A again", externalPath: "/job/L/A_R1" },
            ]
          : [],
    });
  });
  const parsed = await workdayProvider.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
  );
  assert.equal(parsed.length, 1);
  assert.equal(details, 1);
});

test("maximum-page guard and inconsistent pagination fail rather than truncate", async (t) => {
  const tiny = loadProvider({
    "export const WORKDAY_MAX_PAGES = Math.ceil(\n  WORKDAY_MAX_JOBS / WORKDAY_PAGE_SIZE,\n);":
      "export const WORKDAY_MAX_PAGES = 1;",
  }).workdayProvider;
  installWorkdayFetch(t, async () =>
    json({
      total: 21,
      jobPostings: Array.from({ length: 20 }, (_, i) => ({
        title: `Job ${i}`,
        externalPath: `/job/L/J_${i}`,
      })),
    }),
  );
  await assert.rejects(
    () =>
      tiny.parseJobs(careersPage("https://tenant.wd5.myworkdayjobs.com/Site")),
    /safe page limit/,
  );
});

test("non-progressing short page with larger total fails safely", async (t) => {
  installWorkdayFetch(t, async () => json({ total: 10, jobPostings: [] }));
  await assert.rejects(
    () =>
      workdayProvider.parseJobs(
        careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
      ),
    /completion was established/,
  );
});

test("total job limit fails safely", async (t) => {
  installWorkdayFetch(t, async () =>
    json({ total: WORKDAY_MAX_JOBS + 1, jobPostings: [] }),
  );
  await assert.rejects(
    () =>
      workdayProvider.parseJobs(
        careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
      ),
    /safe import limit/,
  );
});

test("bounded detail concurrency is used and output order remains deterministic", async (t) => {
  let active = 0;
  let maxActive = 0;
  installWorkdayFetch(t, async (url, init) => {
    if (init.method === "POST")
      return json({
        jobPostings: Array.from({ length: 12 }, (_, i) => ({
          title: `Job ${i}`,
          externalPath: `/job/L/J_${i}`,
        })),
      });
    active += 1;
    maxActive = Math.max(maxActive, active);
    await new Promise((resolve) => setTimeout(resolve, 5));
    active -= 1;
    return json({
      jobPostingInfo: {
        jobReqId: url.split("_").at(-1),
        jobDescription: "<p>Ok</p>",
      },
    });
  });
  const parsed = await workdayProvider.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
  );
  assert.equal(maxActive, WORKDAY_DETAIL_CONCURRENCY);
  assert.deepEqual(
    parsed.map((job) => job.externalId),
    Array.from({ length: 12 }, (_, i) => String(i)),
  );
});

test("overall deadline fails the complete parse", async (t) => {
  const fastTimeout = loadProvider({
    "export const WORKDAY_LISTING_TIMEOUT_MS = 90_000;":
      "export const WORKDAY_LISTING_TIMEOUT_MS = 1;",
  }).workdayProvider;
  let active = 0;
  let settled = 0;
  installWorkdayFetch(t, async (_url, init) => {
    active += 1;
    try {
      await new Promise((resolve) => setTimeout(resolve, 10));
      return init.method === "POST"
        ? json({
            jobPostings: [{ title: "Slow", externalPath: "/job/L/Slow_R1" }],
          })
        : json({ jobPostingInfo: { jobReqId: "R1" } });
    } finally {
      active -= 1;
      settled += 1;
    }
  });
  await assert.rejects(
    () =>
      fastTimeout.parseJobs(
        careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
      ),
    /timed out/,
  );
  assert.equal(active, 0);
  assert.equal(settled, 1);
});

test("Workday listing and request safety limits retain their bounded values", () => {
  assert.equal(WORKDAY_LISTING_TIMEOUT_MS, 90_000);
  assert.equal(WORKDAY_REQUEST_TIMEOUT_MS, 10_000);
  assert.equal(WORKDAY_LISTING_CONCURRENCY, 8);
  assert.equal(WORKDAY_LISTING_MAX_ATTEMPTS, 3);
  assert.equal(WORKDAY_PAGE_SIZE, 20);
  assert.equal(WORKDAY_MAX_JOBS, 10_000);
  assert.equal(WORKDAY_MAX_PAGES, 500);
  assert.equal((WORKDAY_MAX_PAGES - 1) * WORKDAY_PAGE_SIZE, 9_980);
  assert.equal(WORKDAY_MAX_PAGES * WORKDAY_PAGE_SIZE, 10_000);
  assert.equal(WORKDAY_MAX_RESPONSE_BYTES, 2 * 1024 * 1024);
  assert.equal(WORKDAY_MAX_CUMULATIVE_BYTES, 64 * 1024 * 1024);
  assert.match(
    readFileSync(providerPath, "utf8"),
    /const retryDelaysMs = \[250, 600\] as const;/,
  );
});

test("a Target-sized board can complete after the former listing budget", async (t) => {
  // Scale 45/90 seconds to 450/900 ms so the deadline behavior is deterministic
  // without making the test suite wait for a production-length timeout.
  const scaledDeadlineProvider = loadProvider({
    "export const WORKDAY_LISTING_TIMEOUT_MS = 90_000;":
      "export const WORKDAY_LISTING_TIMEOUT_MS = 900;",
  }).workdayProvider;
  let requests = 0;
  installWorkdayFetch(t, async (_url, init) => {
    requests += 1;
    const offset = JSON.parse(init.body).offset;
    if (offset === 0) await new Promise((resolve) => setTimeout(resolve, 500));
    const count = offset < 2_200 ? 20 : 0;
    return json({
      total: 2_200,
      jobPostings: Array.from({ length: count }, (_, index) => ({
        title: `Job ${offset + index}`,
        externalPath: `/job/L/J_${offset + index}`,
      })),
    });
  });

  const jobs = await scaledDeadlineProvider.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
    { detailMode: "listing" },
  );

  assert.equal(requests, 111);
  assert.equal(jobs.length, 2_200);
  assert.deepEqual(
    jobs.map((job) => job.externalId),
    Array.from({ length: 2_200 }, (_, index) => String(index)),
  );
});

test("per-response and cumulative byte limits fail safely", async (t) => {
  const tiny = loadProvider({
    "export const WORKDAY_MAX_RESPONSE_BYTES = 2 * 1024 * 1024;":
      "export const WORKDAY_MAX_RESPONSE_BYTES = 20;",
    "export const WORKDAY_MAX_CUMULATIVE_BYTES = 64 * 1024 * 1024;":
      "export const WORKDAY_MAX_CUMULATIVE_BYTES = 200;",
  }).workdayProvider;
  installWorkdayFetch(
    t,
    async () =>
      new Response(JSON.stringify({ jobPostings: [] }), {
        headers: { "content-type": "application/json", "content-length": "21" },
      }),
  );
  await assert.rejects(
    () =>
      tiny.parseJobs(careersPage("https://tenant.wd5.myworkdayjobs.com/Site")),
    /too large/,
  );
  const cumulative = loadProvider({
    "export const WORKDAY_MAX_CUMULATIVE_BYTES = 64 * 1024 * 1024;":
      "export const WORKDAY_MAX_CUMULATIVE_BYTES = 120;",
  }).workdayProvider;
  installWorkdayFetch(t, async (_url, init) =>
    init.method === "POST"
      ? json({
          jobPostings: [
            { title: "A", externalPath: "/job/L/A_R1" },
            { title: "B", externalPath: "/job/L/B_R2" },
          ],
        })
      : json({
          jobPostingInfo: { jobReqId: "R", jobDescription: "x".repeat(80) },
        }),
  );
  await assert.rejects(
    () =>
      cumulative.parseJobs(
        careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
      ),
    /cumulative/,
  );
});

test("Target-shaped listing payload evidence supports the bounded 64 MiB budget", async (t) => {
  const totalJobs = 9_999;
  const padding = "x".repeat(4_700);
  let cumulativeBodyBytes = 0;
  let largestBodyBytes = 0;
  installWorkdayFetch(t, async (_url, init) => {
    const offset = JSON.parse(init.body).offset;
    const count = Math.min(WORKDAY_PAGE_SIZE, totalJobs - offset);
    const body = JSON.stringify({
      total: offset === 0 ? totalJobs : 0,
      jobPostings: Array.from({ length: count }, (_, index) => ({
        title: `Job ${offset + index}`,
        externalPath: `/job/L/J_${offset + index}`,
        searchText: padding,
      })),
    });
    const bodyBytes = Buffer.byteLength(body);
    cumulativeBodyBytes += bodyBytes;
    largestBodyBytes = Math.max(largestBodyBytes, bodyBytes);
    return new Response(body, {
      headers: { "content-type": "application/json" },
    });
  });

  const parsed = await workdayProvider.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
    { detailMode: "listing" },
  );

  assert.equal(parsed.length, totalJobs);
  assert.ok(cumulativeBodyBytes > 40 * 1024 * 1024);
  assert.ok(cumulativeBodyBytes < WORKDAY_MAX_CUMULATIVE_BYTES);
  assert.ok(largestBodyBytes < WORKDAY_MAX_RESPONSE_BYTES);
});

test("listing byte accounting excludes headers and unread redirect bodies", async (t) => {
  const tiny = loadProvider({
    "export const WORKDAY_MAX_CUMULATIVE_BYTES = 64 * 1024 * 1024;":
      "export const WORKDAY_MAX_CUMULATIVE_BYTES = 100;",
  }).workdayProvider;
  let calls = 0;
  installWorkdayFetch(t, async () => {
    calls += 1;
    if (calls === 1)
      return new Response("unread redirect body ".repeat(20), {
        status: 302,
        headers: {
          location: "/wday/cxs/tenant/Site/jobs",
          "x-large-header": "h".repeat(500),
        },
      });
    return new Response('{"total":0,"jobPostings":[]}', {
      headers: {
        "content-type": "application/json",
        "x-large-header": "h".repeat(500),
      },
    });
  });

  assert.deepEqual(
    await tiny.parseJobs(
      careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
      { detailMode: "listing" },
    ),
    [],
  );
  assert.equal(calls, 2);
});

test("received retry bytes remain charged and cumulative diagnostics are numeric-only", async (t) => {
  const logs = captureConsoleError(t);
  const tiny = loadProvider({
    "export const WORKDAY_MAX_CUMULATIVE_BYTES = 64 * 1024 * 1024;":
      "export const WORKDAY_MAX_CUMULATIVE_BYTES = 100;",
  }).workdayProvider;
  let attempts = 0;
  installWorkdayFetch(t, async () => {
    attempts += 1;
    if (attempts === 1) {
      let reads = 0;
      return new Response(
        new ReadableStream({
          pull(controller) {
            reads += 1;
            if (reads === 1) controller.enqueue(new Uint8Array(80));
            else
              controller.error(new Error("secret Target URL tenant account"));
          },
        }),
        { headers: { "content-type": "application/json" } },
      );
    }
    return new Response('{"total":0,"jobPostings":[]}', {
      headers: { "content-type": "application/json" },
    });
  });

  await assert.rejects(
    () =>
      tiny.parseJobs(careersPage("https://tenant.wd5.myworkdayjobs.com/Site"), {
        detailMode: "listing",
      }),
    /cumulative/,
  );
  assert.equal(attempts, 2);
  assert.deepEqual(Object.keys(logs[0][0]).sort(), [
    "cumulativeByteLimit",
    "cumulativeBytesReceived",
    "failureCode",
    "numberOfListingRequests",
    "numberOfPages",
    "numberOfRetryAttempts",
    "provider",
    "stage",
  ]);
  assert.deepEqual(logs[0][0], {
    provider: "workday",
    stage: "listing",
    failureCode: "cumulative_budget_exceeded",
    cumulativeBytesReceived: 108,
    cumulativeByteLimit: 100,
    numberOfPages: 0,
    numberOfListingRequests: 2,
    numberOfRetryAttempts: 1,
  });
  assert.doesNotMatch(
    JSON.stringify(logs),
    /Target|URL|tenant|account|myworkdayjobs|\/job\/|stack/i,
  );
});

test("listing and detail hydration receive separate bounded byte budgets", async (t) => {
  const tiny = loadProvider({
    "export const WORKDAY_MAX_CUMULATIVE_BYTES = 64 * 1024 * 1024;":
      "export const WORKDAY_MAX_CUMULATIVE_BYTES = 180;",
  }).workdayProvider;
  installWorkdayFetch(t, async (_url, init) =>
    init.method === "POST"
      ? json({
          jobPostings: [
            {
              title: "A".repeat(75),
              externalPath: "/job/L/A_R1",
            },
          ],
        })
      : json({
          jobPostingInfo: {
            jobReqId: "R1",
            jobDescription: "d".repeat(100),
          },
        }),
  );

  const parsed = await tiny.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
  );
  assert.equal(parsed.length, 1);
  assert.equal(parsed[0].descriptionHtml, "d".repeat(100));
});

test("non-JSON and HTML responses fail safely", async (t) => {
  installWorkdayFetch(
    t,
    async () =>
      new Response("<html>nope</html>", {
        headers: { "content-type": "text/html" },
      }),
  );
  await assert.rejects(
    () =>
      workdayProvider.parseJobs(
        careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
      ),
    /not JSON/,
  );
  installWorkdayFetch(
    t,
    async () =>
      new Response("{", { headers: { "content-type": "application/json" } }),
  );
  await assert.rejects(
    () =>
      workdayProvider.parseJobs(
        careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
      ),
    /response JSON was malformed/,
  );
});

test("redirect policy rejects other hosts and accepts bounded same-host redirects", async (t) => {
  installWorkdayFetch(t, async (url) =>
    url.includes("redirected")
      ? json({ jobPostings: [] })
      : new Response(null, {
          status: 302,
          headers: { location: "https://evil.example/redirected" },
        }),
  );
  await assert.rejects(
    () =>
      workdayProvider.parseJobs(
        careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
      ),
    /unsupported host/,
  );
  installWorkdayFetch(t, async (url) =>
    url.includes("redirected")
      ? json({ jobPostings: [] })
      : new Response(null, {
          status: 302,
          headers: {
            location: "https://tenant.wd5.myworkdayjobs.com/redirected",
          },
        }),
  );
  assert.deepEqual(
    await workdayProvider.parseJobs(
      careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
    ),
    [],
  );
});

test("one failed detail request fails the complete parse to avoid false closures", async (t) => {
  installWorkdayFetch(t, async (_url, init) =>
    init.method === "POST"
      ? json({ jobPostings: [{ title: "A", externalPath: "/job/L/A_R1" }] })
      : new Response("No", { status: 500 }),
  );
  await assert.rejects(
    () =>
      workdayProvider.parseJobs(
        careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
      ),
    /Workday jobs request failed\./,
  );
});

function captureConsoleError(t) {
  const calls = [];
  const previousError = console.error;
  t.after(() => {
    console.error = previousError;
  });
  console.error = (...args) => {
    calls.push(args);
  };
  return calls;
}

async function assertWorkdayFailureLog(provider, handler, expectedLog) {
  globalThis.fetch = handler;
  await assert.rejects(() =>
    provider.parseJobs(
      careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
    ),
  );
  return expectedLog;
}

test("Workday first-page listing failures log only fixed safe diagnostic categories", async (t) => {
  const logs = captureConsoleError(t);
  const tinyResponse = loadProvider({
    "export const WORKDAY_MAX_RESPONSE_BYTES = 2 * 1024 * 1024;":
      "export const WORKDAY_MAX_RESPONSE_BYTES = 20;",
  }).workdayProvider;
  const fastRequestTimeout = loadProvider({
    "export const WORKDAY_REQUEST_TIMEOUT_MS = 10_000;":
      "export const WORKDAY_REQUEST_TIMEOUT_MS = 1;",
    "export const WORKDAY_PARSE_TIMEOUT_MS = 30_000;":
      "export const WORKDAY_PARSE_TIMEOUT_MS = 1_000;",
  }).workdayProvider;
  const fastOverallTimeout = loadProvider({
    "export const WORKDAY_LISTING_TIMEOUT_MS = 90_000;":
      "export const WORKDAY_LISTING_TIMEOUT_MS = 1;",
  }).workdayProvider;

  const cases = [
    [
      workdayProvider,
      async () =>
        new Response("secret status 400 body PapaJohns", { status: 400 }),
      "http_400",
    ],
    [
      workdayProvider,
      async () =>
        new Response("secret status 401 body PapaJohns", { status: 401 }),
      "http_401",
    ],
    [
      workdayProvider,
      async () =>
        new Response("secret status 403 body PapaJohns", { status: 403 }),
      "http_403",
    ],
    [
      workdayProvider,
      async () =>
        new Response("secret status 404 body PapaJohns", { status: 404 }),
      "http_404",
    ],
    [
      workdayProvider,
      async () =>
        new Response("secret status 429 body PapaJohns", { status: 429 }),
      "http_429",
    ],
    [
      workdayProvider,
      async () =>
        new Response("secret status 500 body PapaJohns", { status: 500 }),
      "http_5xx",
    ],
    [
      workdayProvider,
      async () =>
        new Response("secret status 503 body PapaJohns", { status: 503 }),
      "http_5xx",
    ],
    [
      workdayProvider,
      async () =>
        new Response("secret status 418 body PapaJohns", { status: 418 }),
      "http_other",
    ],
    [
      workdayProvider,
      async () => {
        throw new Error("secret network message PapaJohns tenant Site");
      },
      "network_failure",
    ],
    [
      workdayProvider,
      async () =>
        new Response('{"jobPostings": [', {
          headers: { "content-type": "application/json" },
        }),
      "malformed_json",
    ],
    [
      workdayProvider,
      async () =>
        new Response("<html>secret PapaJohns body</html>", {
          headers: { "content-type": "text/html" },
        }),
      "non_json_response",
    ],
    [
      tinyResponse,
      async () =>
        new Response(JSON.stringify({ jobPostings: [] }), {
          headers: {
            "content-type": "application/json",
            "content-length": "21",
          },
        }),
      "response_too_large",
    ],
    [
      workdayProvider,
      async () => json({ total: WORKDAY_MAX_JOBS + 1, jobPostings: [] }),
      "reported_total_too_large",
    ],
    [
      workdayProvider,
      async () => json({ jobPostings: "secret malformed payload" }),
      "malformed_listing",
    ],
    [
      workdayProvider,
      async () =>
        json({
          jobPostings: [
            {
              title: "Secret Cook",
              externalPath: "https://evil.example/job/X/Y_R1",
            },
          ],
        }),
      "invalid_external_path",
    ],
    [
      fastRequestTimeout,
      async (_url, init) => {
        await new Promise((resolve) => setTimeout(resolve, 20));
        if (init.signal.aborted)
          throw Object.assign(new Error("secret abort"), {
            name: "AbortError",
          });
        return json({ jobPostings: [] });
      },
      "request_timeout",
    ],
    [
      fastOverallTimeout,
      async () => {
        await new Promise((resolve) => setTimeout(resolve, 20));
        return json({ jobPostings: [] });
      },
      "overall_timeout",
    ],
  ];

  for (const [provider, handler, failureCode] of cases) {
    await assertWorkdayFailureLog(provider, handler, {
      provider: "workday",
      stage: "listing",
      failureCode,
    });
  }

  assert.deepEqual(
    logs.slice(0, -1).map((args) => args[0]),
    cases.slice(0, -1).map(([, , failureCode]) => ({
      provider: "workday",
      stage: "listing",
      failureCode,
    })),
  );
  assert.deepEqual(
    Object.keys(logs.at(-1)[0]).sort(),
    [
      "authoritativeTotalExtensionCount",
      "cumulativeRequestDurationMs",
      "dynamicallyPlannedPageCount",
      "failureCode",
      "firstPageDurationMs",
      "idleGapDurationMs",
      "initialPlannedPageCount",
      "maximumConcurrentRequestsObserved",
      "numberOfListingRequests",
      "numberOfPages",
      "numberOfRetriedRequests",
      "numberOfRetryAttempts",
      "pageFetchDurationMs",
      "pageSchedulingDurationMs",
      "provider",
      "sentinelExtensionCount",
      "stage",
      "totalListingDurationMs",
      "validationDurationMs",
    ].sort(),
  );
  assert.equal(logs.at(-1)[0].provider, "workday");
  assert.equal(logs.at(-1)[0].stage, "listing");
  assert.equal(logs.at(-1)[0].failureCode, "overall_timeout");
  for (const [key, value] of Object.entries(logs.at(-1)[0]))
    if (!["provider", "stage", "failureCode"].includes(key)) {
      assert.equal(typeof value, "number");
      assert.ok(value >= 0);
    }
  for (const args of logs) {
    assert.equal(args.length, 1);
    assert.doesNotMatch(
      JSON.stringify(args),
      /secret|PapaJohns|Cook|evil\.example|tenant|Site|myworkdayjobs|stack|payload|status 400|status 401|status 403|status 404|status 418|status 429|status 500|status 503|network message/i,
    );
  }
});

test("Workday pagination total instability logs only bounded aggregate totals", async (t) => {
  const logs = captureConsoleError(t);
  installWorkdayFetch(t, async (_url, init) => {
    const offset = JSON.parse(init.body).offset;
    if (offset === 0)
      return json({
        total: 180,
        jobPostings: Array.from({ length: 20 }, (_, i) => ({
          title: `PapaJohns Secret Cook ${i}`,
          externalPath: `/job/Secret/Path_${i}`,
          accountId: `account-${i}`,
        })),
      });
    return json({
      total: 180 + WORKDAY_MAX_TOTAL_DRIFT + 1,
      jobPostings: [
        {
          title: "PapaJohns Secret Manager",
          externalPath: "/job/Secret/Manager_R1",
          accountId: "account-latest",
        },
      ],
    });
  });

  await assert.rejects(() =>
    workdayProvider.parseJobs(
      careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
    ),
  );

  assert.deepEqual(logs, [
    [
      {
        provider: "workday",
        stage: "listing",
        failureCode: "pagination_total_unstable",
        firstReportedTotal: 180,
        minimumReportedTotal: 180,
        maximumReportedTotal: 281,
        latestReportedTotal: 281,
        pagesRequested: 10,
        rawRowsRetrieved: 21,
      },
    ],
  ]);
  assert.doesNotMatch(
    JSON.stringify(logs),
    /PapaJohns|Secret|Cook|Manager|account|tenant|Site|myworkdayjobs|\/job\//i,
  );
});

test("overall timeout diagnostic confirms post-first-page listing concurrency", async (t) => {
  const logs = captureConsoleError(t);
  const fastOverallTimeout = loadProvider({
    "export const WORKDAY_LISTING_TIMEOUT_MS = 90_000;":
      "export const WORKDAY_LISTING_TIMEOUT_MS = 15;",
  }).workdayProvider;
  installWorkdayFetch(t, async (_url, init) => {
    const offset = JSON.parse(init.body).offset;
    if (offset === 0)
      return json({
        total: 1_000,
        jobPostings: Array.from({ length: 20 }, (_, index) => ({
          title: `Private title ${index}`,
          externalPath: `/job/Private/Path_${index}`,
        })),
      });
    await new Promise((resolve) => setTimeout(resolve, 30));
    return json({ total: 1_000, jobPostings: [] });
  });

  await assert.rejects(() =>
    fastOverallTimeout.parseJobs(
      careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
      { detailMode: "listing" },
    ),
  );

  assert.equal(logs.length, 1);
  const diagnostic = logs[0][0];
  assert.equal(diagnostic.failureCode, "overall_timeout");
  assert.equal(diagnostic.numberOfPages, 1);
  assert.equal(diagnostic.numberOfListingRequests, 9);
  assert.equal(
    diagnostic.maximumConcurrentRequestsObserved,
    WORKDAY_LISTING_CONCURRENCY,
  );
  assert.doesNotMatch(
    JSON.stringify(diagnostic),
    /Private|Path|tenant|Site|myworkdayjobs/i,
  );
});

test("Workday pagination and detail failures log distinct safe diagnostic categories", async (t) => {
  const logs = captureConsoleError(t);
  const tinyCumulative = loadProvider({
    "export const WORKDAY_MAX_CUMULATIVE_BYTES = 64 * 1024 * 1024;":
      "export const WORKDAY_MAX_CUMULATIVE_BYTES = 120;",
  }).workdayProvider;
  const tinyPages = loadProvider({
    "export const WORKDAY_MAX_PAGES = Math.ceil(\n  WORKDAY_MAX_JOBS / WORKDAY_PAGE_SIZE,\n);":
      "export const WORKDAY_MAX_PAGES = 1;",
  }).workdayProvider;

  const paged =
    (firstTotal, secondTotal, secondJobs = []) =>
    async (_url, init) => {
      const offset = JSON.parse(init.body).offset;
      if (offset === 0)
        return json({
          total: firstTotal,
          jobPostings: Array.from({ length: 20 }, (_, i) => ({
            title: `Job ${i}`,
            externalPath: `/job/L/J_${i}`,
          })),
        });
      return json({ total: secondTotal, jobPostings: secondJobs });
    };

  const cases = [
    [
      tinyCumulative,
      async (_url, init) =>
        init.method === "POST"
          ? json({
              jobPostings: [
                { title: "Secret A", externalPath: "/job/L/A_R1" },
                { title: "Secret B", externalPath: "/job/L/B_R2" },
              ],
            })
          : json({
              jobPostingInfo: { jobReqId: "R", jobDescription: "x".repeat(80) },
            }),
      { provider: "workday", stage: "detail", failureCode: "detail_failed" },
    ],
    [
      workdayProvider,
      paged(101, 101 + WORKDAY_MAX_TOTAL_DRIFT + 1),
      {
        provider: "workday",
        stage: "listing",
        failureCode: "pagination_total_unstable",
        firstReportedTotal: 101,
        minimumReportedTotal: 101,
        maximumReportedTotal: 101 + WORKDAY_MAX_TOTAL_DRIFT + 1,
        latestReportedTotal: 101 + WORKDAY_MAX_TOTAL_DRIFT + 1,
        pagesRequested: 7,
        rawRowsRetrieved: 20,
      },
    ],
    [
      workdayProvider,
      async () => json({ total: 10, jobPostings: [] }),
      {
        provider: "workday",
        stage: "listing",
        failureCode: "pagination_incomplete",
      },
    ],
    [
      workdayProvider,
      async (_url, init) =>
        JSON.parse(init.body).offset === 0
          ? json({
              jobPostings: Array.from({ length: 20 }, (_, i) => ({
                title: `Job ${i}`,
                externalPath: `/job/L/J_${i}`,
              })),
            })
          : JSON.parse(init.body).offset === 20
            ? json({
                total: 10,
                jobPostings: [
                  { title: "Extra", externalPath: "/job/L/Extra_R1" },
                ],
              })
            : json({ total: 10, jobPostings: [] }),
      {
        provider: "workday",
        stage: "detail",
        failureCode: "detail_failed",
      },
    ],
    [
      tinyPages,
      async () =>
        json({
          total: 41,
          jobPostings: Array.from({ length: 20 }, (_, i) => ({
            title: `Job ${i}`,
            externalPath: `/job/L/J_${i}`,
          })),
        }),
      {
        provider: "workday",
        stage: "listing",
        failureCode: "pagination_limit_exceeded",
        paginationLimitReason: "required_data_offset_over_limit",
      },
    ],
    [
      workdayProvider,
      async (_url, init) =>
        init.method === "POST"
          ? json({ jobPostings: [{ title: "A", externalPath: "/job/L/A_R1" }] })
          : new Response("No", { status: 500 }),
      { provider: "workday", stage: "detail", failureCode: "detail_failed" },
    ],
  ];

  for (const [provider, handler] of cases) {
    globalThis.fetch = handler;
    await assert.rejects(() =>
      provider.parseJobs(
        careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
      ),
    );
  }

  assert.deepEqual(
    logs.map((args) => args[0]),
    cases.map(([, , expected]) => expected),
  );
  for (const args of logs) {
    assert.equal(args.length, 1);
    assert.doesNotMatch(
      JSON.stringify(args),
      /Secret|Extra|\/job\/|tenant|Site|myworkdayjobs|status 500|stack/i,
    );
  }
});

test("successful Workday parsing does not log an error", async (t) => {
  const logs = captureConsoleError(t);
  installWorkdayFetch(t, async (_url, init) =>
    init.method === "POST"
      ? json({
          jobPostings: [{ title: "Host", externalPath: "/job/Towson/Host_R1" }],
        })
      : json({
          jobPostingInfo: { jobReqId: "R1", jobDescription: "<p>Host</p>" },
        }),
  );

  const parsed = await workdayProvider.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
  );
  assert.equal(parsed.length, 1);
  assert.deepEqual(logs, []);
});

test("provider registration includes Workday without removing Greenhouse", () => {
  const source = readFileSync(registryPath, "utf8");
  assert.match(source, /greenhouseProvider, workdayProvider/);
  assert.match(source, /import \{ workdayProvider \}/);
});

test("listing mode previews 100+ Workday jobs without detail requests or descriptions", async (t) => {
  const calls = [];
  const postings = Array.from({ length: 105 }, (_, i) => ({
    title: `Job ${i}`,
    externalPath: `/job/Loc/Job_${i}`,
    locationsText: "Baltimore, MD",
    postedOn: "2026-08-01",
    timeType: "Full_time",
  }));
  installWorkdayFetch(t, async (url, init) => {
    calls.push({ url, method: init.method });
    assert.equal(init.method, "POST");
    const body = JSON.parse(init.body);
    return json({
      total: 105,
      jobPostings: postings.slice(body.offset, body.offset + 20),
    });
  });
  const parsed = await workdayProvider.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
    { detailMode: "listing" },
  );
  assert.equal(parsed.length, 105);
  assert.equal(calls.filter((call) => call.method === "GET").length, 0);
  assert.equal(
    parsed.some((job) => "descriptionHtml" in job),
    false,
  );
  assert.deepEqual(parsed[104], {
    externalId: "104",
    providerKey: "workday",
    sourceUrl: "https://tenant.wd5.myworkdayjobs.com/Site/job/Loc/Job_104",
    title: "Job 104",
    location: "Baltimore, MD",
    employmentType: "Full_time",
    applyUrl: "https://tenant.wd5.myworkdayjobs.com/Site/job/Loc/Job_104",
    updatedAt: "2026-08-01",
  });
});

test("bounded concurrent listing fetches complete Papa Johns-sized board deterministically", async (t) => {
  const offsets = [];
  let active = 0;
  let maxActive = 0;
  installWorkdayFetch(t, async (_url, init) => {
    assert.equal(init.method, "POST");
    const offset = JSON.parse(init.body).offset;
    offsets.push(offset);
    active += 1;
    maxActive = Math.max(maxActive, active);
    await new Promise((resolve) => {
      setTimeout(
        resolve,
        offset === 0 ? 0 : Math.max(0, 10 - (offset % 160) / 20),
      );
    });
    active -= 1;
    const remaining = Math.max(0, 1682 - offset);
    const count = Math.min(20, remaining);
    return json({
      total: offset === 0 ? 1682 : 0,
      jobPostings: Array.from({ length: count }, (_, i) => ({
        title: `Job ${offset + i}`,
        externalPath: `/job/L/J_${offset + i}`,
      })),
    });
  });

  const parsed = await workdayProvider.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
    { detailMode: "listing" },
  );

  assert.equal(parsed.length, 1682);
  assert.ok(maxActive <= WORKDAY_LISTING_CONCURRENCY);
  assert.equal(offsets[0], 0);
  assert.equal(offsets.filter((offset) => offset === 0).length, 1);
  assert.equal(new Set(offsets).size, offsets.length);
  assert.deepEqual(
    [...offsets].sort((a, b) => a - b),
    Array.from({ length: 86 }, (_, i) => i * 20),
  );
  assert.deepEqual(
    parsed.slice(0, 5).map((job) => job.externalId),
    ["0", "1", "2", "3", "4"],
  );
  assert.equal(parsed.at(-1).externalId, "1681");
});

test("listing pool replenishes an early slot and a full sentinel without a batch barrier", async (t) => {
  const started = [];
  const requested = new Map();
  const releases = new Map();
  let active = 0;
  let maxActive = 0;
  const waitForStart = async (offset) => {
    for (
      let attempt = 0;
      attempt < 100 && !started.includes(offset);
      attempt += 1
    )
      await new Promise((resolve) => setTimeout(resolve, 1));
    assert.ok(started.includes(offset), `offset ${offset} did not start`);
  };
  installWorkdayFetch(t, async (_url, init) => {
    const offset = JSON.parse(init.body).offset;
    started.push(offset);
    requested.set(offset, (requested.get(offset) ?? 0) + 1);
    active += 1;
    maxActive = Math.max(maxActive, active);
    if (offset <= 180)
      await new Promise((resolve) => releases.set(offset, resolve));
    active -= 1;
    const count = offset < 180 ? 20 : 0;
    return json({
      total: offset === 0 ? 160 : 0,
      jobPostings: Array.from({ length: count }, (_, i) => ({
        title: `Job ${offset + i}`,
        externalPath: `/job/L/J_${offset + i}`,
      })),
    });
  });

  const parsing = workdayProvider.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
    { detailMode: "listing" },
  );
  await waitForStart(0);
  assert.deepEqual(started, [0]);
  releases.get(0)();
  await waitForStart(160);
  assert.deepEqual(started.slice(1), [20, 40, 60, 80, 100, 120, 140, 160]);
  assert.equal(active, WORKDAY_LISTING_CONCURRENCY);

  // Offset 160 is the original sentinel. Its full page extends the plan, and
  // offset 180 must claim the freed slot while the other seven remain blocked.
  releases.get(160)();
  await waitForStart(180);
  assert.equal(active, WORKDAY_LISTING_CONCURRENCY);
  assert.ok(
    [20, 40, 60, 80, 100, 120, 140].every(
      (offset) => requested.get(offset) === 1,
    ),
  );
  for (const offset of [20, 40, 60, 80, 100, 120, 140, 180])
    releases.get(offset)();

  const parsed = await parsing;
  assert.equal(maxActive, WORKDAY_LISTING_CONCURRENCY);
  assert.deepEqual(
    [...requested.entries()].sort(([a], [b]) => a - b),
    Array.from({ length: 9 + WORKDAY_SENTINEL_LOOKAHEAD_PAGES }, (_, index) => [
      index * 20,
      1,
    ]),
  );
  assert.deepEqual(
    parsed.map((job) => job.externalId),
    Array.from({ length: 180 }, (_, index) => String(index)),
  );
});

test("later larger authoritative total extends the listing plan without shrinking on zero or smaller totals", async (t) => {
  const offsets = [];
  installWorkdayFetch(t, async (_url, init) => {
    const offset = JSON.parse(init.body).offset;
    offsets.push(offset);
    const total =
      offset === 0 ? 40 : offset === 20 ? 60 : offset === 40 ? 0 : 55;
    const count = offset < 60 ? 20 : 0;
    return json({
      total,
      jobPostings: Array.from({ length: count }, (_, i) => ({
        title: `Job ${offset + i}`,
        externalPath: `/job/L/J_${offset + i}`,
      })),
    });
  });

  const parsed = await workdayProvider.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
    { detailMode: "listing" },
  );

  assert.equal(parsed.length, 60);
  assert.deepEqual(
    [...offsets].sort((a, b) => a - b),
    [0, 20, 40, 60],
  );
});

test("multiple late total extensions drain every dynamically planned offset exactly once", async (t) => {
  const offsets = [];
  let active = 0;
  let maxActive = 0;
  installWorkdayFetch(t, async (_url, init) => {
    const offset = JSON.parse(init.body).offset;
    offsets.push(offset);
    active += 1;
    maxActive = Math.max(maxActive, active);
    await new Promise((resolve) =>
      setTimeout(resolve, offset === 20 ? 1 : offset === 40 ? 8 : 2),
    );
    active -= 1;
    const total =
      offset === 0 ? 40 : offset === 40 ? 80 : offset === 60 ? 120 : 0;
    const count = offset < 120 ? 20 : 0;
    return json({
      total,
      jobPostings: Array.from({ length: count }, (_, i) => ({
        title: `Job ${offset + i}`,
        externalPath: `/job/L/J_${offset + i}`,
      })),
    });
  });

  const parsed = await workdayProvider.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
    { detailMode: "listing" },
  );

  assert.deepEqual(
    offsets.sort((a, b) => a - b),
    [0, 20, 40, 60, 80, 100, 120],
  );
  assert.equal(new Set(offsets).size, offsets.length);
  assert.ok(maxActive <= WORKDAY_LISTING_CONCURRENCY);
  assert.deepEqual(
    parsed.map((job) => job.externalId),
    Array.from({ length: 120 }, (_, i) => String(i)),
  );
});

test("early short page followed by later rows fails safely as sparse", async (t) => {
  installPagedWorkdayFetch(
    t,
    new Map([
      [0, { total: 61, count: 20 }],
      [20, { total: 61, count: 1 }],
      [40, { total: 61, count: 20 }],
      [60, { total: 61, count: 1 }],
      [80, { total: 61, count: 0 }],
    ]),
  );

  await assert.rejects(
    () =>
      workdayProvider.parseJobs(
        careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
        { detailMode: "listing" },
      ),
    /early short page/,
  );
});

test("Target-sized Workday board above the former limit completes under bounded listing concurrency", async (t) => {
  const totalJobs = 6_200;
  const offsets = [];
  let active = 0;
  let maxActive = 0;
  installWorkdayFetch(t, async (_url, init) => {
    assert.equal(init.method, "POST");
    const offset = JSON.parse(init.body).offset;
    offsets.push(offset);
    active += 1;
    maxActive = Math.max(maxActive, active);
    await new Promise((resolve) => setTimeout(resolve, offset === 0 ? 0 : 1));
    active -= 1;
    const remaining = Math.max(0, totalJobs - offset);
    const count = Math.min(20, remaining);
    return json({
      total: offset === 0 ? totalJobs : 0,
      jobPostings: Array.from({ length: count }, (_, i) => ({
        title: `Job ${offset + i}`,
        externalPath: `/job/L/J_${offset + i}`,
      })),
    });
  });

  const parsed = await workdayProvider.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
    { detailMode: "listing" },
  );

  assert.equal(parsed.length, totalJobs);
  assert.ok(maxActive <= WORKDAY_LISTING_CONCURRENCY);
  assert.deepEqual(
    [...offsets].sort((a, b) => a - b),
    Array.from({ length: totalJobs / 20 + 1 }, (_, i) => i * 20),
  );
});

test("a 5,000-10,000-job tail uses bounded rolling lookahead instead of serial requests", async (t) => {
  const initiallyReportedJobs = 6_000;
  const actualJobs = initiallyReportedJobs + 28 * WORKDAY_PAGE_SIZE;
  const attempts = new Map();
  let active = 0;
  let maximumActive = 0;
  let tailActive = 0;
  let maximumTailActive = 0;
  installWorkdayFetch(t, async (_url, init) => {
    const offset = JSON.parse(init.body).offset;
    attempts.set(offset, (attempts.get(offset) ?? 0) + 1);
    active += 1;
    maximumActive = Math.max(maximumActive, active);
    if (offset >= initiallyReportedJobs) {
      tailActive += 1;
      maximumTailActive = Math.max(maximumTailActive, tailActive);
    }
    await new Promise((resolve) => setTimeout(resolve, 2));
    active -= 1;
    if (offset >= initiallyReportedJobs) tailActive -= 1;
    const count = Math.min(WORKDAY_PAGE_SIZE, Math.max(0, actualJobs - offset));
    return json({
      total: offset === 0 ? initiallyReportedJobs : 0,
      jobPostings: Array.from({ length: count }, (_, index) => ({
        title: `Job ${offset + index}`,
        externalPath: `/job/L/J_${offset + index}`,
      })),
    });
  });

  const startedAt = performance.now();
  const parsed = await workdayProvider.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
    { detailMode: "listing" },
  );

  assert.equal(parsed.length, actualJobs);
  assert.ok(performance.now() - startedAt < WORKDAY_LISTING_TIMEOUT_MS);
  assert.equal(maximumActive, WORKDAY_LISTING_CONCURRENCY);
  assert.equal(maximumTailActive, WORKDAY_LISTING_CONCURRENCY);
  assert.ok(
    [...attempts.values()].every((numberOfAttempts) => numberOfAttempts === 1),
  );
});

test("a failed concurrent listing page fails the entire listing without returning partial results", async (t) => {
  const logs = captureConsoleError(t);
  installWorkdayFetch(t, async (_url, init) => {
    const offset = JSON.parse(init.body).offset;
    if (offset === 40) return new Response("No", { status: 500 });
    return json({
      total: 61,
      jobPostings: Array.from({ length: offset < 60 ? 20 : 1 }, (_, i) => ({
        title: `Job ${offset + i}`,
        externalPath: `/job/L/J_${offset + i}`,
      })),
    });
  });
  await assert.rejects(
    () =>
      workdayProvider.parseJobs(
        careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
        { detailMode: "listing" },
      ),
    /request failed/,
  );
  assert.deepEqual(logs, [
    [{ provider: "workday", stage: "listing", failureCode: "http_5xx" }],
  ]);
});

test("listing pages retry transient failures at most three times and recover without diagnostics", async (t) => {
  const logs = captureConsoleError(t);
  for (const [failure, failuresBeforeSuccess] of [
    ["timeout", 1],
    ["timeout", 2],
    ["network", 1],
    ["429", 1],
    ["5xx", 1],
  ]) {
    let calls = 0;
    installWorkdayFetch(t, async () => {
      calls += 1;
      if (calls <= failuresBeforeSuccess) {
        if (failure === "timeout")
          throw Object.assign(new Error("timed out"), { name: "AbortError" });
        if (failure === "network") throw new Error("network unavailable");
        if (failure === "429") return new Response("busy", { status: 429 });
        return new Response("unavailable", { status: 503 });
      }
      return json({ total: 0, jobPostings: [] });
    });
    assert.deepEqual(
      await workdayProvider.parseJobs(
        careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
        { detailMode: "listing" },
      ),
      [],
    );
    assert.equal(calls, failuresBeforeSuccess + 1);
  }
  assert.equal(WORKDAY_LISTING_MAX_ATTEMPTS, 3);
  assert.deepEqual(logs, []);
});

test("exhausted listing timeout retries fail once with the fixed safe diagnostic", async (t) => {
  const logs = captureConsoleError(t);
  let calls = 0;
  installWorkdayFetch(t, async () => {
    calls += 1;
    throw Object.assign(new Error("secret timeout"), { name: "AbortError" });
  });
  await assert.rejects(
    () =>
      workdayProvider.parseJobs(
        careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
        { detailMode: "listing" },
      ),
    /timed out/,
  );
  assert.equal(calls, WORKDAY_LISTING_MAX_ATTEMPTS);
  assert.deepEqual(logs, [
    [{ provider: "workday", stage: "listing", failureCode: "request_timeout" }],
  ]);
});

test("permanent HTTP, malformed JSON, and listing validation failures are not retried", async (t) => {
  for (const response of [
    () => new Response("bad", { status: 400 }),
    () => new Response("bad", { status: 401 }),
    () => new Response("bad", { status: 403 }),
    () => new Response("bad", { status: 404 }),
    () =>
      new Response('{"jobPostings": [', {
        headers: { "content-type": "application/json" },
      }),
    () => json({ jobPostings: "invalid" }),
  ]) {
    let calls = 0;
    installWorkdayFetch(t, async () => {
      calls += 1;
      return response();
    });
    await assert.rejects(() =>
      workdayProvider.parseJobs(
        careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
        { detailMode: "listing" },
      ),
    );
    assert.equal(calls, 1);
  }
});

test("a retried offset stays in one worker slot, extends the plan, and is inserted once in order", async (t) => {
  const attempts = new Map();
  let active = 0;
  let maximumActive = 0;
  installWorkdayFetch(t, async (_url, init) => {
    const offset = JSON.parse(init.body).offset;
    attempts.set(offset, (attempts.get(offset) ?? 0) + 1);
    active += 1;
    maximumActive = Math.max(maximumActive, active);
    await new Promise((resolve) => setTimeout(resolve, 1));
    active -= 1;
    if (offset === 20 && attempts.get(offset) === 1)
      return new Response("busy", { status: 503 });
    const count = offset < 60 ? 20 : 0;
    return json({
      total: offset === 0 ? 40 : offset === 20 ? 60 : 0,
      jobPostings: Array.from({ length: count }, (_, index) => ({
        title: `Job ${offset + index}`,
        externalPath: `/job/L/J_${offset + index}`,
      })),
    });
  });
  const parsed = await workdayProvider.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
    { detailMode: "listing" },
  );
  assert.equal(attempts.get(20), 2);
  assert.deepEqual(
    [...attempts.keys()].sort((a, b) => a - b),
    Array.from(
      { length: WORKDAY_SENTINEL_LOOKAHEAD_PAGES + 3 },
      (_, index) => index * WORKDAY_PAGE_SIZE,
    ),
  );
  assert.ok(maximumActive <= WORKDAY_LISTING_CONCURRENCY);
  assert.equal(parsed.length, 60);
  assert.deepEqual(
    parsed.map((job) => job.externalId),
    Array.from({ length: 60 }, (_, index) => String(index)),
  );
});

test("the listing deadline cancels a pending retry delay before another attempt", async (t) => {
  const fastDeadlineProvider = loadProvider({
    "export const WORKDAY_LISTING_TIMEOUT_MS = 90_000;":
      "export const WORKDAY_LISTING_TIMEOUT_MS = 25;",
  }).workdayProvider;
  let calls = 0;
  installWorkdayFetch(t, async () => {
    calls += 1;
    return new Response("busy", { status: 503 });
  });
  await assert.rejects(
    () =>
      fastDeadlineProvider.parseJobs(
        careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
        { detailMode: "listing" },
      ),
    /listing timed out/,
  );
  assert.equal(calls, 1);
});

test("safe diagnostics for listing plan failures expose no board data", async (t) => {
  const logs = captureConsoleError(t);
  const tinyPages = loadProvider({
    "export const WORKDAY_MAX_PAGES = Math.ceil(\n  WORKDAY_MAX_JOBS / WORKDAY_PAGE_SIZE,\n);":
      "export const WORKDAY_MAX_PAGES = 1;",
  }).workdayProvider;
  installWorkdayFetch(t, async () =>
    json({
      total: 41,
      jobPostings: Array.from({ length: 20 }, (_, i) => ({
        title: `Papa Johns Secret ${i}`,
        externalPath: `/job/Secret/J_${i}`,
        accountId: `acct-${i}`,
      })),
    }),
  );
  await assert.rejects(() =>
    tinyPages.parseJobs(
      careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
    ),
  );
  assert.doesNotMatch(
    JSON.stringify(logs),
    /Papa|Secret|acct|tenant|Site|myworkdayjobs|\/job\//i,
  );
});

test("Target-shaped zero-total tail stops extending after an out-of-order terminal page", async (t) => {
  const initiallyReportedJobs = 2_000;
  const actualJobs = 2_581;
  const requested = [];
  installWorkdayFetch(t, async (_url, init) => {
    const offset = JSON.parse(init.body).offset;
    requested.push(offset);
    // Let higher full pages win races while the eventual short page is slow.
    await new Promise((resolve) =>
      setTimeout(resolve, offset === 2_580 ? 15 : offset % 160 === 0 ? 0 : 2),
    );
    const count = Math.min(WORKDAY_PAGE_SIZE, Math.max(0, actualJobs - offset));
    return json({
      total: offset === 0 ? initiallyReportedJobs : 0,
      jobPostings: Array.from({ length: count }, (_, index) => ({
        title: `Job ${offset + index}`,
        externalPath: `/job/L/J_${offset + index}`,
      })),
    });
  });

  const parsed = await workdayProvider.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
    { detailMode: "listing" },
  );

  assert.equal(parsed.length, actualJobs);
  assert.equal(new Set(requested).size, requested.length);
  assert.ok(Math.max(...requested) < WORKDAY_MAX_JOBS);
  assert.deepEqual(
    parsed.map((job) => job.externalId),
    Array.from({ length: actualJobs }, (_, index) => String(index)),
  );
});

test("sentinel lookahead near the boundary plans only safe speculative offsets and ignores empty tail pages", async (t) => {
  const tiny = loadProvider({
    "export const WORKDAY_MAX_PAGES = Math.ceil(\n  WORKDAY_MAX_JOBS / WORKDAY_PAGE_SIZE,\n);":
      "export const WORKDAY_MAX_PAGES = 6;",
  }).workdayProvider;
  const offsets = [];
  installWorkdayFetch(t, async (_url, init) => {
    const offset = JSON.parse(init.body).offset;
    offsets.push(offset);
    const count = offset < 60 ? 20 : 0;
    return json({
      total: offset === 0 ? 40 : 0,
      jobPostings: Array.from({ length: count }, (_, index) => ({
        title: `Job ${offset + index}`,
        externalPath: `/job/L/J_${offset + index}`,
      })),
    });
  });

  const parsed = await tiny.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
    { detailMode: "listing" },
  );

  assert.equal(parsed.length, 60);
  assert.deepEqual(
    offsets.sort((a, b) => a - b),
    [0, 20, 40, 60, 80, 100],
  );
  assert.equal(new Set(offsets).size, offsets.length);
});

test("row-bearing speculative page after a terminal short page remains sparse near the boundary", async (t) => {
  const tiny = loadProvider({
    "export const WORKDAY_MAX_PAGES = Math.ceil(\n  WORKDAY_MAX_JOBS / WORKDAY_PAGE_SIZE,\n);":
      "export const WORKDAY_MAX_PAGES = 6;",
  }).workdayProvider;
  installWorkdayFetch(t, async (_url, init) => {
    const offset = JSON.parse(init.body).offset;
    const count = offset < 60 ? 20 : offset === 80 ? 1 : 0;
    return json({
      total: offset === 0 ? 40 : 0,
      jobPostings: Array.from({ length: count }, (_, index) => ({
        title: `Job ${offset + index}`,
        externalPath: `/job/L/J_${offset + index}`,
      })),
    });
  });

  await assert.rejects(
    () =>
      tiny.parseJobs(careersPage("https://tenant.wd5.myworkdayjobs.com/Site"), {
        detailMode: "listing",
      }),
    /early short page/,
  );
});

test("a short terminal page at the last safe offset completes without an out-of-range sentinel", async (t) => {
  const totalJobs = WORKDAY_MAX_JOBS - 10;
  const offsets = [];
  let active = 0;
  let maximumActive = 0;
  installWorkdayFetch(t, async (_url, init) => {
    const offset = JSON.parse(init.body).offset;
    offsets.push(offset);
    active += 1;
    maximumActive = Math.max(maximumActive, active);
    await new Promise((resolve) => setTimeout(resolve, 0));
    active -= 1;
    const count = Math.min(WORKDAY_PAGE_SIZE, totalJobs - offset);
    return json({
      total: offset === 0 ? totalJobs : 0,
      jobPostings: Array.from({ length: count }, (_, index) => ({
        title: `Job ${offset + index}`,
        externalPath: `/job/L/J_${offset + index}`,
      })),
    });
  });

  const parsed = await workdayProvider.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
    { detailMode: "listing" },
  );

  assert.equal(parsed.length, totalJobs);
  assert.equal(offsets.at(-1), (WORKDAY_MAX_PAGES - 1) * WORKDAY_PAGE_SIZE);
  assert.equal(new Set(offsets).size, offsets.length);
  assert.ok(maximumActive <= WORKDAY_LISTING_CONCURRENCY);
});

test("a full page at offset 9,980 fails with the fixed hard-limit reason", async (t) => {
  const logs = captureConsoleError(t);
  const offsets = [];
  installWorkdayFetch(t, async (_url, init) => {
    const offset = JSON.parse(init.body).offset;
    offsets.push(offset);
    return json({
      jobPostings: Array.from({ length: WORKDAY_PAGE_SIZE }, (_, index) => ({
        title: `Job ${offset + index}`,
        externalPath: `/job/L/J_${offset + index}`,
      })),
    });
  });

  await assert.rejects(
    () =>
      workdayProvider.parseJobs(
        careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
        { detailMode: "listing" },
      ),
    /safe page limit/,
  );
  assert.ok(offsets.includes(9_980));
  assert.equal(new Set(offsets).size, offsets.length);
  assert.deepEqual(logs, [
    [
      {
        provider: "workday",
        stage: "listing",
        failureCode: "pagination_limit_exceeded",
        paginationLimitReason: "full_page_at_hard_limit",
      },
    ],
  ]);
});

test("hydrateJobs fetches detail once per unique selected Workday job with bounded deterministic concurrency", async (t) => {
  const calls = [];
  let active = 0;
  let maxActive = 0;
  installWorkdayFetch(t, async (url, init) => {
    calls.push({ url, method: init.method });
    if (init.method === "POST") {
      const offset = JSON.parse(init.body).offset;
      return json({
        total: 3,
        jobPostings:
          offset === 0
            ? [0, 1, 2].map((i) => ({
                title: `Job ${i}`,
                externalPath: `/job/Loc/Job_${i}`,
                locationsText: "Baltimore, MD",
                timeType: "Full_time",
              }))
            : [],
      });
    }
    active += 1;
    maxActive = Math.max(maxActive, active);
    await new Promise((resolve) => setTimeout(resolve, 5));
    active -= 1;
    const id = /_(\d+)$/.exec(url)?.[1];
    return json({
      jobPostingInfo: {
        title: `Hydrated ${id}`,
        jobReqId: id,
        jobDescription: `<p>${id}</p>`,
      },
    });
  });
  const listing = await workdayProvider.parseJobs(
    careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
    { detailMode: "listing" },
  );
  const results = await workdayProvider.hydrateJobs({
    careersPage: careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
    jobs: [listing[2], listing[0], listing[2], listing[1]],
  });
  assert.equal(calls.filter((call) => call.method === "GET").length, 3);
  assert.ok(maxActive <= WORKDAY_DETAIL_CONCURRENCY);
  assert.deepEqual(
    results.map((result) =>
      result.status === "ready" ? result.job.externalId : result.externalId,
    ),
    ["2", "0", "1"],
  );
  assert.equal(results[0].job.descriptionHtml, "<p>2</p>");
});
