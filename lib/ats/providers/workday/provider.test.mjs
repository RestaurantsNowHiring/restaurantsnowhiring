...
test("normal pagination completes and overlapping pages are deduplicated in first-seen order", async (t) => {
  ...
  installWorkdayFetch(t, async (url, init) => {
    ...
    if (offset === 0)
      return json({
        total: 101,
        jobPostings: Array.from({ length: 100 }, (_, i) => ({
          title: `Job ${i}`,
    ...
    return json({
      total: 101,
      jobPostings: [
        { title: "Duplicate", externalPath: "/job/Loc/Job_99" },
        { title: "Job 100", externalPath: "/job/Loc/Job_100" },
      ],
    });
  });
  const parsed = await workdayProvider.parseJobs(
...
test("Workday first-page listing failures log only fixed safe diagnostic categories", async (t) => {
  ...
  for (const args of logs) {
    ...
    assert.doesNotMatch(
      JSON.stringify(args),
      /secret|PapaJohns|Cook|evil\.example|tenant|Site|myworkdayjobs|stack|payload/i,
    );
  }
});
