...
test("Workday listing and request safety limits retain their bounded values", () => {
  ...
  assert.equal(WORKDAY_MAX_JOBS, 5_000);
  assert.equal(WORKDAY_MAX_PAGES, 250);
...
test("Target-shaped sentinel tail uses bounded rolling lookahead instead of serial requests", async (t) => {
...
test("a full page at the highest safe speculative offset proves the pagination limit", async (t) => {
  const tiny = loadProvider({
    "export const WORKDAY_MAX_PAGES = Math.ceil(\n  WORKDAY_MAX_JOBS / WORKDAY_PAGE_SIZE,\n);":
      "export const WORKDAY_MAX_PAGES = 4;",
  }).workdayProvider;
  const offsets = [];
  installWorkdayFetch(t, async (_url, init) => {
    const offset = JSON.parse(init.body).offset;
