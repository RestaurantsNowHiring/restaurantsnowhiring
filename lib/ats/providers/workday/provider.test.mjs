...
const {
  workdayProvider,
  WorkdayListingUtilizationTracker,
...
test("listing utilization tracker time-weights full, single, and idle intervals", () => {
  let now = 0;
  const tracker = new WorkdayListingUtilizationTracker(() => now);
  tracker.start(0);
  now = 10;
  tracker.setActive(WORKDAY_LISTING_CONCURRENCY);
  now = 30;
  tracker.setActive(1);
  now = 40;
  tracker.setActive(0);
  ...
  assert.deepEqual(tracker.snapshot(), {
    averageConcurrentRequests: 3.4,
    underfilledDurationMs: 10,
    fullUtilizationDurationMs: 20,
    singleRequestDurationMs: 10,
  });
});
...
test("Workday first-page listing failures log only fixed safe diagnostic categories", async (t) => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      "singleRequestDurationMs",
      "stableDrainCycleCount",
      "stage",
      "totalListingDurationMs",
      "underfilledDurationMs",
...
test("timeout diagnostics distinguish initial, authoritative, and sentinel planning", async (t) => {
  const logs = captureConsoleError(t);
  const diagnosticProvider = loadProvider({
    "export const WORKDAY_LISTING_TIMEOUT_MS = 90_000;":
      "export const WORKDAY_LISTING_TIMEOUT_MS = 35;",
  }).workdayProvider;
  installWorkdayFetch(t, async (_url, init) => {
    const offset = JSON.parse(init.body).offset;
    if (offset === 0)
    ...
    if (offset === 20)
      return json({
        ...
        jobPostings: Array.from({ length: 20 }, (_, i) => ({
          title: `Private ${20 + i}`,
          externalPath: `/job/Private/J_${20 + i}`,
        })),
      });
    if (offset === 40)
      return json({
        ...
        jobPostings: Array.from({ length: 20 }, (_, i) => ({
          title: `Private ${40 + i}`,
          externalPath: `/job/Private/J_${40 + i}`,
        })),
      });
    await new Promise((resolve) => setTimeout(resolve, 60));
    return json({ total: 40, jobPostings: [] });
  });

  await assert.rejects(() =>
    diagnosticProvider.parseJobs(
      careersPage("https://tenant.wd5.myworkdayjobs.com/Site"),
      { detailMode: "listing" },
    ),
  );
  ...
  const diagnostic = logs[0][0];
  assert.equal(diagnostic.initialPlannedPageCount, 2);
  assert.equal(diagnostic.dynamicallyPlannedPageCount, 2);
  assert.equal(diagnostic.authoritativeTotalExtensionCount, 1);
  assert.equal(diagnostic.sentinelExtensionCount, 1);
  assert.ok(diagnostic.stableDrainCycleCount >= 0);
  assert.equal(diagnostic.numberOfPages, 3);
  assert.equal(diagnostic.numberOfListingRequests, 4);
});

test("timeout diagnostics count retry attempts and requests separately", async (t) => {
  const logs = captureConsoleError(t);
  const diagnosticProvider = loadProvider({
    "export const WORKDAY_LISTING_TIMEOUT_MS = 90_000;":
      "export const WORKDAY_LISTING_TIMEOUT_MS = 45;",
    "const retryDelaysMs = [250, 600] as const;":
      "const retryDelaysMs = [1, 1] as const;",
  }).workdayProvider;
  ...
  installWorkdayFetch(t, async (_url, init) => {
    ...
    if (offset === 0)
      return json({
        total: 20,
        jobPostings: Array.from({ length: 20 }, (_, i) => ({
          title: `Confidential ${i}`,
          externalPath: `/job/Hidden/J_${i}`,
        })),
      });
    if (offset === 20 && offset20Attempts++ === 0)
      return new Response("private response", { status: 503 });
    await new Promise((resolve) => setTimeout(resolve, 70));
    return json({ total: 20, jobPostings: [] });
  });
  ...
  const diagnostic = logs[0][0];
  assert.equal(diagnostic.numberOfRetryAttempts, 1);
  assert.equal(diagnostic.numberOfRetriedRequests, 1);
  assert.equal(diagnostic.numberOfPages, 1);
  assert.equal(diagnostic.numberOfListingRequests, 3);
  assert.ok(diagnostic.singleRequestDurationMs > 0);
  assert.ok(diagnostic.underfilledDurationMs > 0);
  assert.equal(diagnostic.fullUtilizationDurationMs, 0);
  assert.doesNotMatch(
    JSON.stringify(diagnostic),
...
test("timeout diagnostics count multiple one-page sentinel extensions", async (t) => {
  const logs = captureConsoleError(t);
  const diagnosticProvider = loadProvider({
    "export const WORKDAY_LISTING_TIMEOUT_MS = 90_000;":
      "export const WORKDAY_LISTING_TIMEOUT_MS = 35;",
  ...
  installWorkdayFetch(t, async (_url, init) => {
    ...
    if (offset < 60)
      return json({
        jobPostings: Array.from({ length: 20 }, (_, i) => ({
          title: `Sensitive ${offset + i}`,
          externalPath: `/job/Private/J_${offset + i}`,
        })),
      });
    await new Promise((resolve) => setTimeout(resolve, 60));
    return json({ jobPostings: [] });
  ...
  assert.equal(diagnostic.initialPlannedPageCount, 2);
  assert.equal(diagnostic.dynamicallyPlannedPageCount, 2);
  assert.equal(diagnostic.sentinelExtensionCount, 3);
  assert.equal(diagnostic.authoritativeTotalExtensionCount, 0);
  assert.equal(diagnostic.numberOfPages, 3);
  assert.equal(diagnostic.numberOfListingRequests, 4);
  assert.doesNotMatch(JSON.stringify(diagnostic), /Sensitive|Private|tenant/);
});
...
test("listing pool replenishes an early slot and a full sentinel without a batch barrier", async (t) => {
  ...
  assert.deepEqual(
    [...requested.entries()],
    Array.from({ length: 10 }, (_, index) => [index * 20, 1]),
  );
  assert.deepEqual(
    parsed.map((job) => job.externalId),
