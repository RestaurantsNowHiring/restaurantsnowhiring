...
test("Workday listing and request safety limits retain their bounded values", () => {
  ...
  assert.equal(WORKDAY_MAX_CUMULATIVE_BYTES, 40 * 1024 * 1024);
  assert.match(
...
test("listing pool replenishes an early slot and a full sentinel without a batch barrier", async (t) => {
  ...
  assert.deepEqual(
    ...
    Array.from(
      { length: 9 + WORKDAY_SENTINEL_LOOKAHEAD_PAGES },
      (_, index) => [index * 20, 1],
    ),
  );
  assert.deepEqual(
...
test("sentinel lookahead near the boundary plans only safe speculative offsets and ignores empty tail pages", async (t) => {
  ...
  assert.equal(parsed.length, 60);
  assert.deepEqual(offsets.sort((a, b) => a - b), [0, 20, 40, 60, 80, 100]);
  assert.equal(new Set(offsets).size, offsets.length);
});
...
test("hydrateJobs fetches detail once per unique selected Workday job with bounded deterministic concurrency", async (t) => {
  ...
  installWorkdayFetch(t, async (url, init) => {
    ...
    if (init.method === "POST") {
      ...
      return json({
        total: 3,
        jobPostings: offset === 0 ? [0, 1, 2].map((i) => ({
          title: `Job ${i}`,
          externalPath: `/job/Loc/Job_${i}`,
          locationsText: "Baltimore, MD",
          timeType: "Full_time",
