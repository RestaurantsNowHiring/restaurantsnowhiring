...
const { workdayProvider, WORKDAY_DETAIL_CONCURRENCY, WORKDAY_MAX_TOTAL_DRIFT } =
  loadProvider();
...
test("later Workday zero total with full jobs is ignored and pagination continues to a short page", async (t) => {
  ...
  assert.equal(offsets.length, 85);
  assert.deepEqual(offsets.slice(0, 3), [0, 20, 40]);
  assert.equal(offsets.at(-1), 1680);
...
test("short-page incomplete checks use last authoritative Workday total after ignored zero", async (t) => {
  installPagedWorkdayFetch(
    t,
    new Map([
...
test("listing mode previews 100+ Workday jobs without detail requests or descriptions", async (t) => {
  ...
  installWorkdayFetch(t, async (url, init) => {
    ...
    const body = JSON.parse(init.body);
    return json({ total: 105, jobPostings: postings.slice(body.offset, body.offset + 20) });
  });
  const parsed = await workdayProvider.parseJobs(careersPage("https://tenant.wd5.myworkdayjobs.com/Site"), { detailMode: "listing" });
  assert.equal(parsed.length, 105);
  assert.equal(calls.filter((call) => call.method === "GET").length, 0);
  assert.equal(parsed.some((job) => "descriptionHtml" in job), false);
...
test("hydrateJobs fetches detail once per unique selected Workday job with bounded deterministic concurrency", async (t) => {
  ...
  installWorkdayFetch(t, async (url, init) => {
    calls.push({ url, method: init.method });
    if (init.method === "POST") return json({ total: 3, jobPostings: [0, 1, 2].map((i) => ({ title: `Job ${i}`, externalPath: `/job/Loc/Job_${i}`, locationsText: "Baltimore, MD", timeType: "Full_time" })) });
    active += 1;
    maxActive = Math.max(maxActive, active);
    await new Promise((resolve) => setTimeout(resolve, 5));
    active -= 1;
    const id = /_(\d+)$/.exec(url)?.[1];
    return json({ jobPostingInfo: { title: `Hydrated ${id}`, jobReqId: id, jobDescription: `<p>${id}</p>` } });
  });
  const listing = await workdayProvider.parseJobs(careersPage("https://tenant.wd5.myworkdayjobs.com/Site"), { detailMode: "listing" });
  const results = await workdayProvider.hydrateJobs({ careersPage: careersPage("https://tenant.wd5.myworkdayjobs.com/Site"), jobs: [listing[2], listing[0], listing[2], listing[1]] });
  assert.equal(calls.filter((call) => call.method === "GET").length, 3);
  assert.ok(maxActive <= WORKDAY_DETAIL_CONCURRENCY);
  assert.deepEqual(results.map((result) => result.status === "ready" ? result.job.externalId : result.externalId), ["2", "0", "1"]);
