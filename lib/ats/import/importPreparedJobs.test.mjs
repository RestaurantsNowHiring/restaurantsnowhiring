...
function mockDatabase({ existing = [], insertError, updateError } = {}) {
  const calls = { find: [], insert: [], update: [] };
  const rows = [...existing];
  return {
    calls,
    database: {
      ...
      async insert(payload) { calls.insert.push(payload); return insertError ? { data: null, error: insertError } : { data: { id: `new-${calls.insert.length}` }, error: null }; },
      async update(id, payload) { calls.update.push({ id, payload }); return updateError ? { data: null, error: updateError } : { data: { id }, error: null }; },
    },
  };
}
...
test("rejects provider, identity, required field, and invalid corrections", async () => {
  ...
  const corrections = [{ providerKey: "greenhouse", externalId: "4", city: "Baltimore", state: "XX" }];
