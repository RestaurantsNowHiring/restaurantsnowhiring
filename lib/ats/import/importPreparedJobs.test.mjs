...
function loadImport(buildCanonicalJobInsertPayload = canonicalPayload, shouldAutoApproveJob = () => false) {
  ...
  const require = (specifier) => {
    ...
    if (specifier === "../../jobPersistence") return { buildCanonicalJobInsertPayload, shouldAutoApproveJob };
    if (specifier === "../../billing") return { syncSubscriptionQuantityForEmployer: async () => {} };
...
test("reuses the canonical job insert payload builder", async () => {
  ...
  assert.equal(received.employerAccountId, "account-1");
  assert.equal(received.approvalDate.toISOString(), "2026-08-03T12:00:00.000Z");
});

test("ATS inserts preserve canonical account-based auto-approval fields", async () => {
  const service = loadImport((values) => ({
    ...canonicalPayload(values),
    ...(values.employerAccountId === "account-1" ? {
      active: true, status: "active", approved_at: values.approvalDate.toISOString(), expires_at: "2026-09-02T12:00:00.000Z",
    } : {}),
  }));
  const db = mockDatabase();
  await service(input([ready("auto")]), deps(db.database));
  assert.equal(db.calls.insert[0].active, true);
  assert.equal(db.calls.insert[0].status, "active");
  assert.equal(db.calls.insert[0].approved_at, "2026-08-03T12:00:00.000Z");
  assert.equal(db.calls.insert[0].source_type, "ats");
  assert.equal(db.calls.insert[0].ats_external_job_id, "auto");
});

test("ATS auto-approval performs the normal best-effort billing quantity sync once", async () => {
  const service = loadImport(canonicalPayload, (accountId) => accountId === "account-1");
  const db = mockDatabase();
  const synced = [];
  await service(input([ready("bill-1"), ready("bill-2")]), {
    ...deps(db.database),
    syncSubscriptionQuantityForEmployer: async (ownerId) => { synced.push(ownerId); },
  });
  assert.deepEqual(synced, ["owner-1"]);
});
