...
const companyLoader = fs.readFileSync("lib/companyPages.ts", "utf8");
...
test("company inventory loader preserves its complete query result type", () => {
  assert.match(companyLoader, /getPublicCompanyInventory\(\): Promise<PublicCompanyJob\[\]>/);
  assert.match(companyLoader, /\.returns<PublicCompanyJob\[\]>\(\)/);
  assert.match(companyLoader, /job\.source_type === "employer"/);
  assert.doesNotMatch(companyDetail, /job: PublicCompanyJob/);
});
