...
test("outreach-free jobs never enter normal auto-renewal", () => {
  assert.equal(isEligibleForAutoRenewal({ ...eligible, source_type: "outreach_free" }, now), false);
  assert.equal(isEligibleForAutoRenewal({ ...eligible, source_type: "rnh_sourced" }, now), false);
  assert.equal(isEligibleForAutoRenewal({ ...eligible, source_type: "employer", ats_provider: "workday" }, now), true);
});
...
test("public expiration visibility is isolated to outreach_free", () => {
  ...
  assert.match(visibleFunction, /sourceType === "outreach_free" && expiresAt/);
});
