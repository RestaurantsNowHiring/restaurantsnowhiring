...
| `jobs.expires_at` | Nullable `timestamptz`, indexed. Initial approval sets approval + 30 days; manual renew sets now + 30 days; pause/resume leave it unchanged. It is currently **informational and scheduling data**, used for dashboard status, reminder selection, and the expiration RPC; it is not the direct visibility gate on the principal public surfaces. The expiration RPC selects it at/before database `now()`. |
...
Dashboard-only `Expired` is **not persisted**. `dashboardStatusForJob` derives it when a persisted `active` or `paused` job has `expires_at <= now`; an active job can therefore display “Expired” before the daily cron persists `paused`. `isPubliclyVisibleJob` checks only persisted `status` and `active`, not `expires_at`. The homepage (`app/page.tsx`), main jobs listing (`app/jobs/page.tsx`), company job data (`lib/companyPages.ts`), and job-detail/slug lookups (`app/jobs/[id]/page.tsx`) use that status/active predicate for their principal public-job selection. They therefore do not provide a reliable exact-timestamp expiration gate. Some secondary discovery surfaces use `isNonExpiredPublicJob` (for example sitemap, role/state landing data, and state-count generation), so public behavior is inconsistent across surfaces. Admin status has no `Expired` representation.
...
* Principal public visibility and billable counting both depend on status/active and do **not** check approval, `expires_at`, source type, or trial state (`lib/jobStatus.ts`, `lib/billing.ts`). Therefore an expired row remains billable and may remain publicly visible between its exact expiry and the cron pause. Secondary surfaces that call `isNonExpiredPublicJob` may suppress the same row during that window, producing inconsistent visibility.
...
* At the exact timestamp, the dashboard can derive `Expired`, but no synchronous status write, universal public-visibility change, or charge occurs. Because `isPubliclyVisibleJob` ignores the date, an expired job that is still `status='active',active=true` may continue to appear on the homepage, main jobs listing, company pages, and job-detail routes. The exact visibility window runs from `expires_at` until the daily cron successfully persists the pause—normally up to roughly 24 hours, but potentially longer if the cron is delayed or fails. Date-aware secondary surfaces may hide it sooner.
* Daily cron later invokes `pause_expired_job_ads()`, which atomically updates every `status='active' AND active=true AND expires_at<=now()` row to `active=false,status='paused'`. That persisted status/active transition—not `expires_at` by itself—is the general public-visibility gate. The RPC neither deletes nor changes `expires_at`, `approved_at`, or ATS reason. The route then sends auto-pause notices and best-effort resynchronizes each collected owner's Stripe quantity.
...
* Daily ATS sync (`0 12 * * *` UTC) updates content. Missing remote jobs become `archived`/inactive with `closed_in_ats`. If they return, only `closed_in_ats` rows reopen: approved rows become active immediately; never-approved rows return pending. Reopening does not change `expires_at`, verify billing, or synchronize Stripe quantity, so an approved expired ATS job can be made `active` with a stale past expiration and can reappear on status/active-only public surfaces until another lifecycle write pauses it (`lib/ats/sync/syncEmployerAtsConnection.ts`). Employer/admin intentional inactive reasons are preserved.
...
Because the expiration route's reminder selection is by entire **UTC date**, the email can be almost six calendar days or less than five exact 24-hour periods before `expires_at`. The persisted pause normally occurs up to roughly 24 hours after exact expiry and can occur later after a cron failure. During that interval status/active-only public surfaces may continue showing the job, even though the dashboard labels it expired and date-aware secondary surfaces may omit it. Both a Vercel schedule and optional database schedule are documented, presenting a deployment audit requirement.
...
| SEO/public job pages | `app/sitemap.ts`, `app/page.tsx`, `app/jobs/page.tsx`, `app/jobs/[id]/page.tsx`, `app/[roleSlug]/page.tsx`, `lib/companyPages.ts`, `lib/jobStatus.ts` | Visibility logic is inconsistent: principal homepage/list/company/detail paths use status/active-only `isPubliclyVisibleJob`, while sitemap and some role/state/count paths use date-aware `isNonExpiredPublicJob`. Auto-renew must define one authoritative visibility transition rather than assume `expires_at` is universally enforced. |
...
* Define and enforce an authoritative visibility transition in backend lifecycle writes. Renewal must not rely on existing public selectors to hide or retain a job at `expires_at`; until selectors are deliberately unified, status/active changes remain necessary to control all principal surfaces.
...
| Expired jobs remain publicly visible until cron | Treat `expires_at` as a renewal/pause scheduling boundary, not a universal query gate; make the worker's status transition reliable and observable, define failure behavior, and deliberately unify date-aware/status-only selectors if exact-timestamp suppression becomes a requirement. |
...
* The post-expiration/pre-cron window: status/active-only public surfaces continue to include the row until the pause persists, while date-aware secondary surfaces omit it; future behavior is tested against the approved authoritative visibility rule.
